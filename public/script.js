class SciFiTerminal {
    ...
    handleContentScroll(event, mode) {
        ...
        if (nearBottom && !this.currentFileInfo.eof) {
            // Request next page
            const lastPage = Math.max(...Array.from(this.currentFileInfo.loadedPages.keys()));
            ...
            const firstPage = Math.min(...Array.from(this.currentFileInfo.loadedPages.keys()));
    ...
    updatePageSize() {
        if (this.currentFileInfo && this.currentFileInfo.filename) {
            const newPageSize = this.calculatePageSize(this.currentFileInfo.mode === 'binary');
    ...
    handleFileSystemMessage(message) {
        ...
        if (message.type === 'file_system' && message.action === 'contents') {
            ...
            console.log('Received file content:', message.filename, message.mode, message);
            
            if (message.pages) {
                // Handle chunked content with pages
                console.log('Handling chunked file content with pages:', message.pages.length);
    ...
    openFile(filename) {
        if (this.isConnected && this.websocket.readyState === WebSocket.OPEN) {
            ...
            // Show loading popup
            this.showFileViewer(filename, 'loading', null, fileItem.size);
    ...
    getFileItemByName(filename) {
        ...
        if (this.currentDirectoryContents) {
            const fileItem = this.currentDirectoryContents.find(item => item.name === filename);
            if (fileItem) {
                return {
                    name: fileItem.name,
    ...
    calculatePageSize(isBinary) {
        ...
        const contentHeight = popupRect.height * 0.8 - 60; // Subtract header height
        const contentWidth = popupRect.width * 0.8;
        ...
        const pageSize = charsPerLine * numberOfLines;
        
        // For binary mode, adjust for hex representation (2 chars per byte + space)
        if (isBinary) {
            const hexCharsPerLine = Math.floor((contentWidth / 2) / (charWidth * 3)); // 2 hex chars + space
            const hexPageSize = hexCharsPerLine * numberOfLines;
            return Math.max(hexPageSize, 1024); // Minimum 1KB
        }
        
        return Math.max(pageSize, 1024); // Minimum 1KB
    }
    ...
    showFileViewer(filename, mode, content, size) {
        ...
        if (mode === 'loading') {
            ...
            // Show binary content in hex viewer
    ...
    handleChunkedFileContent(message) {
        try {
            ...
            // Set mode and pageSize if not already set
            if (!this.currentFileInfo.mode) {
                this.currentFileInfo.mode = message.mode;
                console.log('Set mode to:', message.mode);
            }
            if (!this.currentFileInfo.pageSize) {
                this.currentFileInfo.pageSize = this.calculatePageSize(message.mode === 'binary');
                console.log('Set page size to:', this.currentFileInfo.pageSize);
            }
            ...
            if (message.mode === 'text') {
                ...
                this.displayChunkedTextContent(message);
            } else if (message.mode === 'binary') {
                console.log('Displaying binary content');
                this.displayChunkedBinaryContent(message);
            }
            
            // Update popup to show content instead of loading
            this.showFileViewer(message.filename, message.mode, null, message.totalSize);
    ...
    displayChunkedTextContent(message) {
        if (!this.currentFileInfo || !this.currentFileInfo.loadedPages) {
            console.error('currentFileInfo not properly initialized');
            return;
        }
        ...
        document.getElementById('text-display').textContent = fullContent;
    }

    displayChunkedBinaryContent(message) {
        if (!this.currentFileInfo || !this.currentFileInfo.loadedPages) {
            console.error('currentFileInfo not properly initialized');
            return;
        }
    ...
    requestFileChunk(byteOffset, asPage) {
        if (!this.currentFileInfo || !this.currentFileInfo.filename || !this.isConnected) return;
