        ...
        try {
            if (platform === "linux") {
                ...
                if (commandExists("sw_vers")) {
                    const { stdout: productVersion } = await execAsync(
...
wss.on("connection", (ws) => {
    ...
    ws.send(JSON.stringify({ type: "io", action: "stdout", content: welcomeMessage + "\r\n" }));

    // Handle data from terminal process
    ptyProcess.onData((data) => {
        ws.send(JSON.stringify({ type: "io", action: "stdout", content: data }));
    });
    ...
    ws.on("message", async (message) => {
        try {
            ...
            if (parsed.type === "io") {
                if (parsed.action === "stdin") {
                    ptyProcess.write(parsed.content);
                }
                ...
                } else if (parsed.action === "change_directory") {
            ...
            // If parsing fails, it"s an invalid message type or corrupted JSON, log an error
            console.error("Received non-JSON or malformed message:", message.toString(), e);
            // Optionally send an error back to the client if the message type is expected but malformed
